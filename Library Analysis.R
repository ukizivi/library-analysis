...
data_model <- customers %>% 
  dplyr::inner_join(data_model, by = c("id" = "patron_id")) %>%
  dplyr::mutate(city = tools::toTitleCase(tolower(gsub("\\s+", " ", city))), # remove wherever thes is more than one white space
...
data_model %>%
  dplyr::group_by(book_pages_range) %>%
  dplyr::summarise(number_of_checkouts = sum(!is.na(late_return)), proportion_late = mean(late_return == TRUE, na.rm = TRUE)) %>%
  dplyr::arrange(desc(proportion_late)) %>%
  dplyr::filter(!is.na(book_pages_range))
...
data_model <- data_model %>%
  dplyr::mutate(portland_postal_code = customer_postal_code %in% portland_postal_codes)
...
data_model %>%
  dplyr::filter(!is.na(customer_education) & !is.na(book_pages_range)) %>%
